...
//	ADXRS450_Gyro gyro = new ADXRS450_Gyro(); // Creates new ADXRS450_Gyro variable "gyro".
	...
	boolean agiDelay = true;
	boolean driveToggle = true; // Creates new boolean variable "driveToggle".
	...
	double zAxis;
	double driveAdjuster = 0.115; //+ is left, - is right
		double centerx = 0.0;
	...
	public void robotInit() { // Initialization Stuff
		new Thread(() -> { // Code for cameras.
			UsbCamera camera = CameraServer.getInstance().startAutomaticCapture(); // Creates "camera".
			camera.setResolution(140, 140); // Sets "camera"'s resolution to 320x240.
		}).start(); // Starts it.
//		double[] centerX;
...
//		SmartDashboard.putNumber("table", centerX);
		...
		AutonSwitch.addDefault("MiddleGear", "MiddleGear"); // Adds object "MiddleGear" to SmartDashboard.
		AutonSwitch.addObject("Shoot", "Shoot"); // Adds object "Shoot" to SmartDashboard.
...
//*****************************************************************************************************************************************************				
			if (Stick1.getRawButton(5) == false && driveToggle == false && Stick1.getRawButton(1) == false) { // Tank mode.
				if(normalDrive){
...
//*****************************************************************************************************************************************************				
			if (Stick1.getRawButton(5) == true && driveToggle == false && Stick1.getRawButton(1) == false){//Tonk mode but slow
				myRobot.tankDrive(-.5 * Stick1.getRawAxis(1), -.5 * Stick1.getRawAxis(5)); // Speed of robot (-1/100%).
			}
//*****************************************************************************************************************************************************				
			if (Stick1.getRawButton(5) == false && driveToggle == true && Stick1.getRawButton(1) == false) { // Left stick arcade.
//				double gearIndicatorButton = Stick1.getRawButton(3) ? 1:0.0;
				if(normalDrive){
					myRobot.arcadeDrive(-1 * Stick1.getRawAxis(1), -.85 * Stick1.getRawAxis(0));
				}
				else{
...
//*****************************************************************************************************************************************************				
			if (Stick1.getRawButton(5) == true && driveToggle == true && Stick1.getRawButton(1) == false) { //Left Stick arcade but slow
				myRobot.arcadeDrive(-.5 * Stick1.getRawAxis(1), -.65 * Stick1.getRawAxis(0)); // Speed of robot (-1/50%).
			}
//*****************************************************************************************************************************************************				
			if (Stick1.getRawButton(1) == true){ //Backwards slow drive on second controller for climb
				myRobot.arcadeDrive(.5 * Stick2.getRawAxis(1), -.65 * Stick2.getRawAxis(0));
			}
//*****************************************************************************************************************************************************

			if (Stick1.getRawButton(8) == true) { // Tank/arcade toggle
...
/**			if(Stick1.getRawButton(1) == false){
				climbMotor.set(1 * Stick1.getRawAxis(2));
			} **/
	...
	public void autonomous() { // Autonomous code.
		...
		case "LeftGear": /* "LeftGear" mode, drives forward, turns toward gear loader, places gear, backs up, */ /** turns and drives toward loading station**/
			...
			Timer.delay(1.8);
			myRobot.arcadeDrive(0, -.47);
			Timer.delay(.83);
			myRobot.arcadeDrive(0.6, driveAdjuster);
			Timer.delay(1.15);
			myRobot.arcadeDrive(-1,driveAdjuster);
			Timer.delay(.25);
			myRobot.arcadeDrive(.6, driveAdjuster);
			Timer.delay(.5);
			myRobot.arcadeDrive(0, 0);
		...
		case "MiddleGear": /* drives forward, places gear, backs up, */ /** drives to boiler and shoots **/
			myRobot.arcadeDrive(0.6, driveAdjuster + .04);
			Timer.delay(1.5);			
			myRobot.arcadeDrive(-1, driveAdjuster + .1);
			Timer.delay(.25);
			myRobot.arcadeDrive(.5, (driveAdjuster + .1));
			Timer.delay(1.75);
			myRobot.arcadeDrive(0, 0);
/**			while (gearDetector.get() == false); {
				myRobot.arcadeDrive(0, 0);
	...
	public void shooter(boolean shoot) { // Shooter Code
		...
		if (shoot) {
			shooterMotor.set(1);// Shooter spin speed.
			if(agiDelay){
				Timer.delay(1);
			}
			agitatorMotor.set(1);
